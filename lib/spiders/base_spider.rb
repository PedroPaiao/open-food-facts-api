  ...
  class OpenFoodFactsApi
    ...
    def self.needs(field)
      ...
      define_method field do
        if parameter_object.is_a?(Hash) && parameter_object.key?(field)
          ...
          raise NoMethodError,
                "Missing method: #{field.inspect} for the parameter object called for class: #{self.class.name}"
        end
      end
    end
    ...
    def call
      if steps.present?
        ...
        raise NoMethodError,
              "Called undefined #call. You need either define steps or
              implement the #call method in the class: #{self.class.name}"
      end
    end
    ...
    def process_steps
      ...
      steps.map(&:to_s).each do |step_name|
        ...
        return result if @success == false
      end
      result
    end
    ...
    def process_step(step_name)
      if respond_to?(step_name, true)
        ...
        begin
          ...
          if step_name.starts_with?('return_') && respond_to?(step_name.sub('return_', ''), true)
            send(step_name.sub('return_', ''))
          else
            raise NoMethodError,
                  "Cannot find step implementation for <#{step_name}>.
                  Step should be either a private instance method of #{self.class.name} or camel_case'd name of another
